...
mask  = (target_action.N>0) & (target_action.roundName=='Deal') & target_action.playerName.isin(target_players) #& (target_action.winMoney>0) #
...
X  = target_action.loc[mask,['smallBlind','roundName','chips','position','cards','board','pot','bet','N','Nnf','Nallin','pot_sum','bet_sum','maxBet','NMaxBet','prev_action','Nfold','Ncall','Nraise','NRfold','NRcall','NRraise','self_prev_action','self_Ncall','self_Nraise','self_NRcall','self_NRraise']].copy() #,'score0','score1'
...
X.drop(['smallBlind','roundName','chips','position','cards','board','pot','bet','pot_sum','bet_sum','maxBet','prev_action','self_prev_action',],'columns',inplace=True)
...
model.fit(X,y.action=='fold')
yhat  = model.predict(X)
accuracy_score(y.action=='fold',yhat)
...
for i,(idx_tr,idx_tt) in enumerate(kf.split(X,y.action)):
    ...
    #
    model.fit(X_tr,y_tr.action=='fold')
    yhat_tr  = model.predict_proba(X_tr)[:,1]>0.4
    yhat_tt  = model.predict_proba(X_tt)[:,1]>0.4
    #
